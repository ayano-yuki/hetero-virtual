...
# ローカル生成物の整理とignore設定を追加

workspace直下に残っていたローカル生成物を削除し、再生成されるcache/storeをgit管理対象外として明示した。

## 実装詳細

- `.pnpm-store/`と`.turbo/`を`.gitignore`へ追加
- `apps/demo-next/.next/`を削除
- `apps/demo-next/tsconfig.tsbuildinfo`を削除
- `examples/next-basic/tsconfig.tsbuildinfo`を削除
- `node_modules/`、`evidence/`、`TASKS.md`、`Todo.md`、`LOG.md`は意図的に維持
...
# トップレベルのファイル構造を役割別に刷新

library、demo、playground、documentsの役割別トップレベル構造へ移行し、benchmark環境と証跡をplayground配下へ集約した。

## 実装詳細

- `packages/core`と`packages/react`を`library/`配下へ移動
- `examples/next-basic`を`demo/next-basic`へ移動
- `apps/demo-next`を`playground/benchmark`へ移動し、package名を`benchmark-playground`へ変更
- `apps/private-trial`を`playground/private-trial`へ移動
- `docs`を`documents`へ、`evidence`を`playground/evidence`へ移動
- `pnpm-workspace.yaml`、root scripts、lockfile、README、Todo、TASKS、documents内参照を新構造へ更新
- 移動後にworkspace依存リンクを再作成し、`pnpm test`、`pnpm typecheck`、`pnpm build`で検証
...
完了済みの`Todo.md`と`TASKS.md`を削除し、今後も必要な運用ルールだけを`documents/Rule.md`へ集約した。

## 実装詳細

- `documents/Rule.md`を追加し、North Star、repository roles、API方針、package names、publish rule、Not Nowを整理
- 完了済みチェックリストである`Todo.md`と`TASKS.md`を削除
- READMEのdocumentation linksとpublish-gate参照を`documents/Rule.md`へ更新
- `LOG.md`内の過去履歴としての`Todo.md`/`TASKS.md`参照は維持
...
# library package構造を責務別に整理

coreとreactのpackage内部を公開library向けの責務別ディレクトリへ分割し、test配置とTypeScript設定を更新した。

## 実装詳細

- `library/core/src`を`adapter`、`anchor`、`estimator`、`measurement`、`range`、`scheduler`、`structures`へ分割
- coreのunit testを`library/core/tests`へ集約
- `library/react/src`を`hooks`と`store`へ分割
- reactのunit testを`library/react/tests`へ移動
- 新しい配置に合わせてexport、import、tsconfigのinclude設定を更新
- `pnpm test`、`pnpm typecheck`、`pnpm build`で検証
...
# library内部参照をalias化しpackage READMEを追加

coreとreactの内部importを責務別aliasへ統一し、各packageの構造と運用ルールをREADMEに明記した。

## 実装詳細

- workspace共通のTypeScript pathsとして`@core/*`と`@react/*`を追加
- core内部のimport/exportを`@core/*`へ統一
- react内部のimport/exportを`@react/*`へ統一
- Vitest configにpackage source aliasを追加し、consumer側のtestでも解決できるように更新
- `library/core/README.md`にdirectory structure、責務、import rule、test ruleを追加
- `library/react/README.md`にdirectory structure、責務、import rule、public boundaryを追加
- `pnpm test`、`pnpm typecheck`、`pnpm build`で検証
...
- `library/core/**`
- `library/react/**`
- `playground/benchmark/vitest.config.ts`
- `playground/private-trial/vitest.config.ts`
- `LOG.md`
